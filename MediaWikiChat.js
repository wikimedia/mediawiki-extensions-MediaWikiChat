...
var MediaWikiChat = {
	...
	greyscale: function( element, microseconds ) {
		...
		if ( hours > 1 ) {
			if ( hours > 24 ) {
				tooltip = mw.message( 'chat-idle-more', mw.user ).text();
			} else {
				tooltip = mw.message( 'chat-idle-hours', Math.round( hours ), mw.user ).text();
			}
			hours = 1;
		} else {
			var minutes = microseconds / 6000;
			if ( minutes > 10 ) {
				tooltip = mw.message( 'chat-idle-minutes', Math.round( minutes ), mw.user ).text();
			} else {
