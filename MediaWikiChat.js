...
$( function() {
	...
	function chatDialogBlock( config ) {
		chatDialogBlock.parent.call( this, config );
	}
	OO.inheritClass( chatDialogBlock, OO.ui.ProcessDialog );

	chatDialogBlock.static.name = 'chatDialogBlock';
	chatDialogBlock.static.title = mw.msg( 'chat-dialog-block-message', mw.user.getName() ); // Wrong user
	...
	chatDialogBlock.prototype.initialize = function () {
		chatDialogBlock.parent.prototype.initialize.apply( this );
	...
	var chatBlock = new chatDialogBlock( { size: 'medium' } );

	var windowManager = new OO.ui.WindowManager();
	$( 'body' ).append( windowManager.$element );

	windowManager.addWindows( [ chatBlock ] );
	$( 'body' ).on( 'click', '.mwchat-useritem-blocklink', function( e ) {
		windowManager.openWindow( chatBlock );
		return false;
	} );
